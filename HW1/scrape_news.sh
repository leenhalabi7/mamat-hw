	...
	for links in $new_articles; do
		...
		num_B=$(echo "$current_site" | grep -o "Bennett" | wc -l)
		num_P=$(echo "$current_site" | grep -o "Peretz" | wc -l)
		#zero appearance of the given words case 
		if [[($num_N -eq 0) && ($num_G -eq 0) && ($num_B -eq 0) && ($num_P -eq 0) ]]; then 
			echo "$links, -" >> results.csv  
			continue
		fi
		#prints the results to the results.csv file
		echo "$links,Netanyahu,$num_N,Gantz,$num_G,Bennett,$num_B,Peretz,$num_P" >> results.csv
